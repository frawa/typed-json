(function (typedjson) {
    ...
    class LineView extends ContentView {
        ...
        merge(from, to, source, takeDeco, openStart, openEnd) {
            ...
            mergeInlineChildren(this, from, to, source ? source.children : none$2$1, openStart, openEnd);
            return true;
        }
        split(at) {
...
<button class=${themeClass("button")} type=submit>${view.state.phrase("go")}</button>`;
    ...
    const gotoLine = view => {
        ...
        if (!panel) {
            view.dispatch({
                reconfigure: view.state.field(dialogField, false) == null ? { append: [panels(), dialogField, baseTheme$1] } : undefined,
  ...
  "type": "boolean"
    ...
    let typedJsonSchema = typedjson.TypedJsonFactory
        .withMetaSchema()
        .forValue(initialSchema);
    let typedJson = typedjson.TypedJsonFactory
        .create()
    ...
    function autocompleteConfig(getTypedJson) {
        return {
    ...
    function linterFun(getTypedJson) {
        return (view) => {
